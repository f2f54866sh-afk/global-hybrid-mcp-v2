...
def evidence(*, surface="CONTROLLED_LOCAL_EDIT", task="real car plate cover", revision="gpt-image-2.5-sunburst-2026-09-08"):
...
def test_invalid_spatial_binding_blocks_before_side_effect():
    port = ControlledPort()
    receipt = m.ImageSurfaceController(port).execute(
        local_spec(env=envelope(binding_valid=False))
    )
    assert receipt.state is m.ImageExecutionState.BLOCKED
...
def test_insufficient_geometry_confidence_blocks_before_side_effect():
    port = ControlledPort()
    receipt = m.ImageSurfaceController(port).execute(
        local_spec(env=envelope(confidence_sufficient=False))
    )
    assert receipt.state is m.ImageExecutionState.BLOCKED
        ...
        local_spec().model_copy(
            update={
                "side_effect_budget": budget(
                    attempt=2, limit=2, explicit=True, pending=3, override=True
                )
            }
        )
    )
