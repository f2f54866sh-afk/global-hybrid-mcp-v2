...
def _reserve(store, *, attempt_id=None, number=1, authorization_id=None, prior=None):
    return store.reserve_image_attempt(
        ...
        prior_terminal_result_id=prior,
    )
...
def test_retry_requires_exact_sequential_terminal_binding_and_single_use_authorization(tmp_path):
    ...
    assert (
        _reserve(
            ...
            number=2,
            authorization_id="current-user-auth-2",
            prior="result-1",
        )
        == 2
    )
...
def _image_spec(budget):
    return ImageTaskSpec(
        ...
        reference_set=ReferenceSet(identity_reference=["source-a"]),
...
def _request(spec, *, context=(), bound=False):
    return TaskRequest(
        request_text="make image",
        intent=Intent.EXECUTION,
        effects=[EffectType.IMAGE_GENERATE],
        target_system="image",
        action_class="generate",
        image_task=spec.model_dump(mode="json"),
        context=list(context),
        runtime_state_required=bound,
        conversation_or_thread_id="thread-a" if bound else None,
...
def test_retry_requires_firewall_admitted_current_user_instruction_before_port_call(tmp_path):
    ...
    store.complete_image_attempt(
        ...
        "source-a",
        attempt_id="first",
        terminal_result_id="first-result",
        terminal_status="FAIL",
    )
    port = _RecordingPort()
    budget = ImageSideEffectBudget(
        ...
        explicit_user_authorization_receipt="not-executable",
...
def test_reserved_attempt_cannot_complete_or_refund_quota(tmp_path):
    ...
    with pytest.raises(RuntimeStateError, match="IMAGE_ATTEMPT_COMPLETION_LIFECYCLE_MISMATCH"):
        store.complete_image_attempt(
            ...
            terminal_result_id="result-forbidden",
            terminal_status="PASS",
        )
...
def test_sqlite_durability_pragmas_are_delete_and_extra(tmp_path):
    path = tmp_path / "runtime.db"
    store = SQLiteRuntimeStateStore(path)
...
def test_protected_image_can_initialize_missing_runtime_state_only_once(tmp_path):
    store = SQLiteRuntimeStateStore(tmp_path / "runtime.db")
    port = _RecordingPort()
    request = _request(
        _image_spec(ImageSideEffectBudget(source_asset_id="source-a")),
        context=[_capability()],
        bound=True,
    ).model_copy(update={"runtime_state_initialize": True})

    first = _dispatcher(port, store).dispatch(request)
    assert first.status == "PASS"
    assert port.calls == 1
    initialized = store.load("thread-a", "task-a")
    assert initialized.conversation_or_thread_id == "thread-a"
    assert initialized.task_id == "task-a"

    second = _dispatcher(port, store).dispatch(request)
    assert second.status == "RUNTIME_STATE_ALREADY_INITIALIZED"
    assert port.calls == 1


def test_protected_image_initialization_race_fails_closed(tmp_path, monkeypatch):
    store = SQLiteRuntimeStateStore(tmp_path / "runtime.db")
    store.create(_state(thread="thread-a", task="task-a"))
    monkeypatch.setattr(
        store,
        "load",
        lambda *_args: (_ for _ in ()).throw(RuntimeStateNotFound("simulated race")),
    )
    port = _RecordingPort()
    request = _request(
        _image_spec(ImageSideEffectBudget(source_asset_id="source-a")),
    ...
    result = _dispatcher(port, store).dispatch(request)
    assert result.status == "RUNTIME_STATE_ALREADY_INITIALIZED"
    assert port.calls == 0


def test_stale_runtime_row_cannot_be_reinitialized(tmp_path):
    store = SQLiteRuntimeStateStore(tmp_path / "runtime.db")
    store.create(_state(thread="thread-a", task="task-a"))
    with store._connect() as connection:
        connection.execute(
            "UPDATE runtime_task_state SET runtime_state_version=999 "
...
def test_non_image_request_cannot_initialize_missing_runtime_state(tmp_path):
    store = SQLiteRuntimeStateStore(tmp_path / "runtime.db")
    request = TaskRequest(
        request_text="ordinary execution request",
        intent=Intent.EXECUTION,
        runtime_state_required=True,
        runtime_state_initialize=True,
        conversation_or_thread_id="thread-a",
        runtime_task_id="task-a",
    )
    result = _dispatcher(_RecordingPort(), store).dispatch(request)
    assert result.status == "RUNTIME_STATE_LOAD_BLOCKED"
    with pytest.raises(RuntimeStateError):
        store.load("thread-a", "task-a")


def test_live_execution_kill_switch_blocks_before_image_port(tmp_path):
    store = SQLiteRuntimeStateStore(tmp_path / "runtime.db")
    store.create(_state(thread="thread-a", task="task-a"))
    port = _RecordingPort()
    dispatcher = Dispatcher(
        ...
        runtime_state_store=store,
        image_controller=ImageSurfaceController(port),
        effect_gate=EffectGate(live_execution=False),
    )
