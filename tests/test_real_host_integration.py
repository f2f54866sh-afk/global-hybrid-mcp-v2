...
class _Domain:
    ...
    def run(self, contract):
        ...
        referent = contract.resolved_referent_id
    ...
    *,
    ...
    return TaskRequest(
        ...
        current_identity_projection=CurrentIdentityProjection(
            ...
            identities=identities or dict(CURRENT_HOST_MAPPING),
...
def _dispatcher(domain: _Domain, trace: TraceBus | None = None) -> Dispatcher:
    return Dispatcher(
...
def test_stale_or_wrong_current_mapping_blocks_before_owner_routing(capsys):
    ...
    wrong = _dispatcher(_Domain()).dispatch(
        _request(identities={**CURRENT_HOST_MAPPING, "執行長": "GLOBAL"}),
        require_host_projection=True,
    )
    assert wrong.status == HOST_STATE_MAPPING_MISMATCH
    events = [json.loads(line) for line in capsys.readouterr().out.splitlines()]
...
def test_mcp_host_transport_requires_current_projection_and_is_stateless():
    ...
    assert "dispatch_host_task" in {tool.name for tool in tools.tools}
